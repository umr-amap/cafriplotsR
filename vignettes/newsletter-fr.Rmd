...
Trois améliorations plus modestes accompagnent ce changement :

- **Les identifiants `.Renviron` sont désormais détectés automatiquement.** Si vous aviez exécuté `setup_db_credentials()` pour conserver `MYDB_USER` et `MYDB_PASS`, vous n'avez plus besoin du drapeau `use_env_credentials = TRUE` — le package les détecte et se connecte sans invite. Passez `use_env_credentials = FALSE` pour forcer l'invite à chaque appel.
- **Le nom d'utilisateur est demandé avant le mot de passe**, comme dans tout formulaire de connexion classique.
- **Un mot de passe erroné ne reste plus en cache.** Auparavant, une faute de frappe restait stockée en mémoire et chaque tentative de connexion suivante la réutilisait jusqu'à découvrir `reset = TRUE`. Les échecs d'authentification effacent désormais le cache et redemandent les identifiants automatiquement en mode interactif ; le comportement de réessai avec délai pour les erreurs réseau transitoires est conservé.
