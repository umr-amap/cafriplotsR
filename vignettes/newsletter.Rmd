...
Three smaller quality-of-life improvements ship alongside:

- **`.Renviron` credentials are now picked up automatically.** If you previously ran `setup_db_credentials()` to persist `MYDB_USER` and `MYDB_PASS`, you no longer need the `use_env_credentials = TRUE` flag — the package detects them and connects without prompting. Pass `use_env_credentials = FALSE` to opt out and always be prompted.
- **Username is asked before password**, matching standard login forms.
- **A wrong password no longer sticks.** Previously a typo was cached in memory and every subsequent connect attempt reused it until users discovered `reset = TRUE`. Authentication failures now clear the cache and re-prompt automatically in interactive sessions; transient network errors keep the existing retry-with-backoff behaviour.
