...
Les colonnes WCVP ne constituent pas l'un de ces groupes : elles sont ajoutées dès lors que l'option WCVP était activée avant la correspondance, et accompagnent l'export dans tous les cas. L'identifiant de ligne interne `id_data` est toujours retiré.
...
Lorsque l'option WCVP est activée (voir [Intégration WCVP](#intégration-wcvp)), quatre colonnes supplémentaires sont ajoutées et `corrected_name` est **remplacé** par le nom WCVP partout où il en existe un :
...
**Elle réécrit également `corrected_name`.** Partout où WCVP contient le taxon, `corrected_name` devient le nom WCVP plutôt que celui du référentiel interne ; les taxons absents de WCVP conservent leur nom interne. La colonne `name_source` enregistre laquelle des deux références a fourni chaque valeur (`internal` ou `WCVP`), ce qui rend la substitution vérifiable — consultez-la avant de considérer que `corrected_name` provient d'une référence unique.
