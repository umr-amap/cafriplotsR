...
Le cas standard : Un spécimen a été collecté sur cet arbre individuel pendant le recensement de parcelle. Le spécimen sert de témoin taxonomique pour l'arbre.

### `type_individual`
Cas spécial : L'individu est le **spécimen type** réel (holotype, isotype, etc.) pour une description d'espèce. C'est rare mais scientifiquement critique.

**Comment c'est déterminé :**
- L'application vérifie si la colonne `herbarium_nbe_type` correspond à `herbarium_nbe_char`
- Si oui → `type_individual`
- Si non → `referenced_individual`
...
├─ herbarium_nbe_type (référence du spécimen type)
...
3. **Documentez précisément les types de liens** : Distinguez entre les témoins réguliers et les spécimens types.
