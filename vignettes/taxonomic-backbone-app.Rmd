---
title: "Managing the Taxonomic Backbone with the Interactive App"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Managing the Taxonomic Backbone with the Interactive App}
...
The `launch_taxo_backbone_app()` function provides an interactive Shiny application for managing the Central African plant taxonomic backbone database. This application allows you to:

- **Browse and search** taxonomic entries at all levels (family, genus, species, etc.)
- **Visualize taxonomic hierarchy**
- **Add new taxa** either manually, or by retrieving information from the TROPICOS database or from a taxonomic backbone (WCVP, APD, ...)
...
**Search Options:**
- **Exact match**: Strict matching (case-insensitive)
- **Fuzzy match** (default): Similarity-based matching for typos
- **Include synonyms of queried taxa**: Also show taxa that are synonyms of your search term
- **Include child taxa**: Show all descendants (e.g., all species in a genus)

**Synonymy Filter:**
- **All taxa**: Shows both accepted names and synonyms
- **Accepted names only**: Filters out synonyms (idtax_good_n IS NULL)
- **Synonyms only**: Shows only synonym entries

#### Structured Search
Use separate fields for genus, species, family, order for more precise queries.
...
- **Backbone links**: every external backbone registered in the database (WCVP, the African Plant Database, ...) is listed for the selected taxon, with the name it points at and whether that link is the one supplying the taxon's name. A link whose match was fuzzy, or whose authors disagreed, is shown as *awaiting review* and supplies no name until someone accepts it
...
**Features:**
- **Breadcrumb path**: Shows the lineage (Class → Order → Family → Genus → Species)
- **Tree visualization**: Nested view with color-coded levels
- **Children summary**: Counts of genera, species, infraspecific taxa under the selected taxon
- **ID display**: Shows database IDs for reference

**Use Cases:**
- Verify that a species is correctly placed in its genus
- Check the full lineage of a taxon
...
**Workflow:**
1. Fill in taxonomic information (genus, species, family, etc.)
2. Set taxonomic level (family, genus, species, infraspecific)
3. The app **automatically**:
   - Finds the appropriate parent taxon
   - Creates it if missing (e.g., creates genus entry when adding a species)
   - Links via id_parent
   - Validates the hierarchy
   
The name is searched in **every backbone the database registers at once** - WCVP, the African Plant Database, and any other - and in TROPICOS when a Tropicos API key is available. A name usually exists in several backbones, and its identifier is worth keeping in each of them, so the search is not a choice between backbones. If no match is found anywhere, the new entry can be filled manually.

Searching does three things beyond filling the form:

- Every backbone whose match is **unambiguous** - an exact match on the name searched for, and the only one in that backbone - is selected automatically. Step 1 and step 4 list exactly which identifiers will be linked, and *Clear all backbone links* drops them.
- **Use backbone result** fills the form from the selected row *and* records that row's backbone link; **Validate backbone match only** keeps the form as it is and records the link alone. Either overrides what was selected automatically for that same backbone, and leaves the other backbones untouched. Links are saved once the taxon is created, marked verified, since a person chose them.
- If other taxa of the internal backbone already point at the same accepted name in any of the selected backbones, they are listed once each, with the evidence of every backbone that suggested them, and step 5 offers to set them as synonyms of the new entry.
...
1. **New species awaiting formal description**: Include the (future) author name if known
   Example: _Strephonema sp.nov. Lachenaud O._ (where "sp.nov." is the species epithet)

2. **Locality-restricted morpho-taxon**: Add the locality name in the species epithet
   Example: _Diospyros sp.nov. Sao Tomé Nguema_
...
**Important Notes:**
- Always provide complete upper taxonomy
- Use correct spelling and nomenclatural authorities
- Check for duplicates before adding (this is checked in any case)
...
- Change species epithet, author, publication year
- Update non-hierarchical fields
...
1. **Detects descendants** via id_parent (all children, grandchildren, etc.)
...
**NA Value Handling:**
The app safely handles NA values in all fields, preventing crashes.
...
**Important:**
- Prevents synonym chains (A → B → C)
- All synonyms must point directly to an accepted name
...
5. **Missing Parents**: Taxa with upper fields filled but no id_parent link
...
**In the App:**
- View inconsistencies by type
- See affected taxa with details
- Fix automatically or manually

**Example Issues:**
- Family "Asteraceae" has tax_order = "Asterales" but id_parent = NULL
- Species "Coffea arabica" has tax_gen = "Coffea" but parent is a family entry
...
**Features:**
- View existing traits for selected taxon
- Add new trait measurements
- Link to trait definitions
...
**Best Practice:**
1. Identify the **correct/primary** entry
2. Make duplicates **synonyms** of the primary
3. Use "Accepted names only" filter to work with the primary
...
- **Consistency validation**: Check that species → genus → family → order relationships are correct
- **Flexible updates**: Cascade changes when modifying upper taxa

**The app automatically maintains synchronization between both systems.**
...
2. Run diagnostics: `db_diagnostic()`
