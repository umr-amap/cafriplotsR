...
#' Delete an entry in traits measurements table (DEPRECATED)
#'
#' @description
#' **DEPRECATED**: Use [safe_delete_taxa_traits()] instead.
#'
#' This function is unsafe because it does not delete related records in
#' `taxa_traits_measures_feat` first, which may violate foreign key constraints.
#'
#' @param id integer vector of id_trait_measures values to delete
...
.delete_entry_sp_trait_measure <- function(id, con = NULL) {
  ...
  .Deprecated(
    new = "safe_delete_taxa_traits",
    msg = paste0(
      "`.delete_entry_sp_trait_measure()` is deprecated and unsafe.\n",
      "Use `safe_delete_taxa_traits(id_trait_measures = ...)` instead,\n",
      "which also removes related records in `taxa_traits_measures_feat`."
    )
  )
