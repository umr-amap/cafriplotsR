...
**Date**: 2025-10-20 **Status**: Proposal - Ready to Implement

## Problem Statement

The `table_taxa` table currently lacks an explicit field indicating the
taxonomic level of each entry. This creates several issues:
...
**Problems with this approach:** 1. **Ambiguous**: Empty `tax_esp` could
mean missing data OR genus-level taxon 2. **Fragile**: Relies on data
entry consistency 3. **Verbose**: Requires multiple conditions for each
query 4. **Error-prone**: Easy to forget edge cases (infraspecific taxa,
etc.)
...
Populate with one of these values: - `"infraspecific"` - Taxa with
infraspecific components (variety, subspecies, etc.) - `"species"` -
Species-level taxa (has genus + species epithet) - `"genus"` -
Genus-level taxa (has genus only) - `"family"` - Family-level taxa (has
family only) - `"higher"` - Higher taxonomic levels (order, class,
phylum, etc.)
...
Queries filtering by `tax_level` will be faster than multi-column
checks.
...
This will: 1. Check if column exists 2. Add `tax_level` column to
`table_taxa` 3. Populate values based on logic above 4. Create index for
performance 5. Verify results with summary statistics 6. Show examples
of each level 7. Test queries

### Step 2: Update Batch Matching Code

Replace the batch matching logic in `R/mod_auto_matching.R` with the
updated version from `mod_auto_matching_UPDATED_with_tax_level.R`.
...
Distribution of levels makes sense (most should be
...
can use tax_level for filtering
...
2.  **`mod_auto_matching_UPDATED_with_tax_level.R`** - Updated batch
    matching code
...
This is safe because the original data is not modified - we only added a
new column.
...
**Improve clarity** - Explicit intent in code ✅ **Boost performance** -
Indexed column for faster filtering ✅ **Reduce errors** - Less
ambiguity about taxonomic levels ✅ **Enable validation** - Easy to spot
data quality issues

**Recommendation**: Implement this change. The benefits far outweigh the
small implementation effort.

------------------------------------------------------------------------

**Next Steps**: 1. Review this proposal 2. Run `add_tax_level_field.R`
script 3. Verify results 4. Update `mod_auto_matching.R` with cleaner
filtering 5. Gradually update other functions to use `tax_level`

------------------------------------------------------------------------

**Author**: Claude Code Assistant **Date**: 2025-10-20 **Status**: Ready
to implement
