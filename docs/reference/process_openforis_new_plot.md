...
- individuals:
  ...
  (tag of the main stem, NA for the main stem itself).
  `herbarium_nbe_char` is repeated on every individual identified as the
  species of a voucher, while `herbarium_nbe_type` names each specimen
  once, on the individual it was collected from. `idtax_n` is copied
  from the OpenForis `species_code` without being checked against the
  taxonomic backbone — see the note below.
