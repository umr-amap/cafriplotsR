...
This function creates: 1. A PostgreSQL function \`add_person()\` that
validates and inserts new people 2. Grants EXECUTE permission on this
function to PUBLIC (all users) 3. The function runs with SECURITY
DEFINER (owner's privileges)

Users can then add people by calling: “\`sql SELECT
add_person('FirstName', 'LastName', 'Nationality', 'Institute',
'Contact'); “\`

The function validates that: - First name and last name are provided and
non-empty - The person doesn't already exist (based on full name)
