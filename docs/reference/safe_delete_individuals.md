...
This function safely deletes one or more individuals and all related
data in the correct order to respect foreign key constraints: 1.
Individual measurement features 2. Trait measurements 3. Individuals

\*\*Plot metadata is preserved\*\* (plot, subplots, and subplot features
remain intact).

\*\*Safety features:\*\* - Dry-run mode to preview what will be
deleted - Shows counts of all related data - Requires explicit
confirmation (unless force = TRUE) - Uses database transaction (rolls
back on error) - Detailed logging of each step
...
- individual_ids:
  ...
  Integer vector. Specific individual ID(s) to delete (id_n from
  data_individuals). If NULL, deletes ALL individuals in the specified
  plot(s). Default NULL.

- tags:

  Numeric vector. Specific tag numbers to delete. Alternative to
  individual_ids. If provided along with plot_name, deletes individuals
  matching those tags in those plots. Default NULL.
