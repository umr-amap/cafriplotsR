...
This vignette demonstrates how to add **subplot features** (census
information, team members, plot characteristics, etc.) to existing plots
in the database using the user-friendly
...
stored in the `data_liste_sub_plots` table: - **People**: Team leader,
principal investigator, data manager, etc. - **Dates**: Census dates
(year, month, day) - **Plot characteristics**: Various plot-specific
...
Your data should have: 1. A **plot identifier column** (`plot_name` or
`id_liste_plots`) 2. One or more **feature columns**
...
The function automatically maps column names, but you can also provide
custom mappings:
...
For people-related features, you can specify multiple people using
...
Common synonyms recognized: - `PI`, `princ_invest`, `investigator` →
`principal_investigator` - `teamlead`, `team_lead`, `leader` →
`team_leader` - `datamanager`, `data_mgr`, `manager` → `data_manager` -
...
**Solution**: Ensure plot names exactly match database values. Use
`exact_match = TRUE` when querying:
...
**Solution**: The function will prompt you to add new people
interactively. Alternatively, add people first:
...
2.  **Verify plot names**: Ensure plot names in your data exactly match
    database values

3.  **Use interactive mode initially**: Let the function help you map
    columns interactively

4.  **Save mappings for reuse**: If you have a standard data format,
...
5.  **Verify after import**: Always query back the data to confirm
    features were added correctly

6.  **Handle people carefully**: For people-related features, ensure
    names are formatted consistently (e.g., “First Last” or “Last,
    First”)
...
- [`query_plots()`](https://umr-amap.github.io/cafriplotsR/reference/query_plots.md) -
  Query plot metadata and features
...
The
[`add_plot_features()`](https://umr-amap.github.io/cafriplotsR/reference/add_plot_features.md)
function provides a user-friendly interface for adding subplot features
to existing plots:

1.  ✅ **Intelligent column mapping** (exact, synonym, fuzzy,
    interactive)
...
6.  ✅ **Safe operations** (uses existing add_subplot_features
    internally)

Always start with a dry run, verify the mapping, then proceed with the
actual import!
