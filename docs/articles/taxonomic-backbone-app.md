...
- **Add new taxa** either manually, or by retrieving information from
  the TROPICOS database or from a taxonomic backbone (WCVP, APD, …)
...
- **Backbone links**: every external backbone registered in the database
  (WCVP, the African Plant Database, …) is listed for the selected
  taxon, with the name it points at and whether that link is the one
  supplying the taxon’s name. A link whose match was fuzzy, or whose
  authors disagreed, is shown as *awaiting review* and supplies no name
...
The name is searched in **every backbone the database registers at
once** - WCVP, the African Plant Database, and any other - and in
TROPICOS when a Tropicos API key is available. A name usually exists in
several backbones, and its identifier is worth keeping in each of them,
so the search is not a choice between backbones. If no match is found
anywhere, the new entry can be filled manually.

Searching does three things beyond filling the form:

- Every backbone whose match is **unambiguous** - an exact match on the
  name searched for, and the only one in that backbone - is selected
  automatically. Step 1 and step 4 list exactly which identifiers will
  be linked, and *Clear all backbone links* drops them.
- **Use backbone result** fills the form from the selected row *and*
  records that row’s backbone link; **Validate backbone match only**
  keeps the form as it is and records the link alone. Either overrides
  what was selected automatically for that same backbone, and leaves the
  other backbones untouched. Links are saved once the taxon is created,
  marked verified, since a person chose them.
- If other taxa of the internal backbone already point at the same
  accepted name in any of the selected backbones, they are listed once
  each, with the evidence of every backbone that suggested them, and
  step 5 offers to set them as synonyms of the new entry.
