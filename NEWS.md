...
* **`backbone_citation()` states how to cite the names a query used** (`R/backbone_core.R`, `inst/migrations/backbone_citation_metadata.R`) — APD asks to be cited with its version and the date its data were obtained, and a name read from a backbone is a name borrowed from its publisher. The citation is built from the database — publisher and homepage from `backbone_list`, version and date from `backbone_import` — so it is the citation of *that* database's names rather than a sentence held in the package
  ...
  - `inst/migrations/backbone_citation_metadata.R` adds `backbone_list.homepage` and writes the publisher, homepages and APD version. **Until it runs, `backbone_citation("apd")` states the wrong publisher and no site**
