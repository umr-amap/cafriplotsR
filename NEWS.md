...
* **Taxa added from the taxonomic backbone app were left outside the hierarchy** (`R/taxonomic_update_functions.R`, `R/taxonomic_query_functions.R`, `R/taxa_hierarchy_links.R`) — `.add_taxa_noninteractive()`, the "Add taxon" step of `launch_taxo_backbone_app()`, wrote the flat columns (`tax_fam`, `tax_gen`, `tax_esp`...) and neither `tax_level` nor `id_parent`. The hierarchy view, `get_taxon_ancestors()` and `get_taxon_children()` walk `id_parent` only, so a new species showed alone and was missing from its genus's children; `check_hierarchy_consistency()` could not report it either, every one of its checks filtering on `tax_level`. Checked on the taxa database: every recent `tax_source = 'NEW'` row had both columns NULL
  ...
  - **Taxa already added need a backfill:** `inst/migrations/taxa_hierarchy_backfill.R`, not yet applied. `migrate_link_unlinked_taxa(con)` rehearses by doing the work in a transaction and rolling it back, so it prints exactly what `dry_run = FALSE` will do; `check_unlinked_taxa(con)` counts what is left
